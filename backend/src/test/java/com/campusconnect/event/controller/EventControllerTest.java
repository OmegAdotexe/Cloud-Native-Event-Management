...
class EventControllerTest {
    ...
    void ownerCanUpdateButAnotherAdminCannot() throws Exception {
        Long id = createEvent();
        mockMvc.perform(patch("/api/events/{id}", id).header("Authorization", bearer(adminToken)).contentType(MediaType.APPLICATION_JSON).content("{\"venue\":\"New Hall\"}"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.venue").value("New Hall"));
        User other = userRepository.save(user("other@test.com", Role.EVENT_ADMIN));
        mockMvc.perform(patch("/api/events/{id}", id).header("Authorization", bearer(jwtService.generateToken(other))).contentType(MediaType.APPLICATION_JSON).content("{\"venue\":\"Nope\"}"))
                .andExpect(status().isForbidden());
    }
    ...
    void ownerCanCancelAndDeleteEvent() throws Exception {
        Long id = createEvent();
        mockMvc.perform(patch("/api/events/{id}/cancel", id).header("Authorization", bearer(adminToken)))
    ...
    void participantOnlySeesPublishedEvents() throws Exception {
        eventRepository.save(Event.builder().title("Draft").venue("A").startTime(LocalDateTime.of(2030, 6, 1, 10, 0)).endTime(LocalDateTime.of(2030, 6, 1, 11, 0)).capacity(10).status(EventStatus.DRAFT).createdBy(admin).build());
        eventRepository.save(Event.builder().title("Published").venue("B").startTime(LocalDateTime.of(2030, 6, 2, 10, 0)).endTime(LocalDateTime.of(2030, 6, 2, 11, 0)).capacity(10).status(EventStatus.PUBLISHED).createdBy(admin).build());
    ...
    private String validRequest() { return "{\"title\":\"Robotics Workshop\",\"description\":\"Build robots\",\"venue\":\"Lab 204\",\"startTime\":\"2030-06-01T10:00:00\",\"endTime\":\"2030-06-01T12:00:00\",\"capacity\":30}"; }
}
